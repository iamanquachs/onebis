...
$list_donhang = $db->load_print_donhang($msdv, $soct);
$list_tongcong = $db->load_print_tongcong($msdv, $soct);
    ...
    <table class="min-w-full">
        ...
        <tbody>
            ...
            foreach ($list_donhang as $r) {
                ...
                $giamgia += $r->giamgia;
                if ($r->nhom_hh == 'VC') {
                    $giamgiacovoucher = $giamgia + abs($r->thanhtien);
                    $sotienvoucher = abs($r->thanhtien);
                }
                if ($r->nhom_hh != 'VC') {
            ...
            <tr>
                ...
                <td class='border text-[12px] text-end font_semi px-2 py-1' style="text-align: end;"><?= str_replace(',', '.', number_format($r->nhom_hh == 'VC' ? $giamgiacovoucher : $giamgia)) ?></td>
            </tr>
            <tr>
                <td class='border text-[12px] text-end font_semi px-2 py-1' style="text-align: end;" colspan="3">Thanh toán</td>
                <td class='border text-[12px] text-end font_semi px-2 py-1' style="text-align: end;"><?= str_replace(',', '.', number_format($tongcong - $giamgiacovoucher)) ?></td>
            </tr>
            <tr>
                <td class='border text-[12px] text-end font_semi px-2 py-1' style="text-align: end;" colspan="3">Đã thanh toán</td>
                <td class='border text-[12px] text-end font_semi px-2 py-1' style="text-align: end;"><?= str_replace(',', '.', number_format(($list_tongcong[0]->dathanhtoan - $sotienvoucher))) ?></td>
            </tr>
            <tr>
