...
class dichvu extends database_sv
{
    ...
    public function delete_lieutrinh($mshs, $msdv, $msdichvu, $mslieutrinh)
    {
        ...
        DELETE FROM motadichvu where  msdichvu ='$msdichvu' and  mslieutrinh='$mslieutrinh'  and mshs='$mshs' and msdv ='$msdv' ");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    public function load_chitiet_lieutrinh($mshs, $msdv, $msdichvu, $mslieutrinh)
    {
        $getall = $this->connect->prepare("SELECT rowid,msdichvu,mslieutrinh,mshh,tenhh, soluong, round(dongia * soluong) as dongia, dinhluong, ptthuchien from motadichvu where msdichvu ='$msdichvu' and mslieutrinh='$mslieutrinh' and mshs='$mshs' and msdv='$msdv'  order by tenhh");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    public function load_chitiet_lieutrinh_tuvan($mshs, $msdv, $mslieutrinh, $msdichvu)
    {
        $getall = $this->connect->prepare("SELECT rowid,msdichvu,mslieutrinh,mshh,tenhh, soluong, dongia, dinhluong from motadichvu where  msdichvu ='$msdichvu' and  mslieutrinh='$mslieutrinh' and mshs='$mshs' and msdv='$msdv' and left(mshh,2)<>'HH' order by tenhh");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    ...
    {
        $getall = $this->connect->prepare("SELECT rowid,msdichvu, mslieutrinh,mshh,tenhh, soluong, round(dongia * soluong,0) as dongia, dinhluong, ptthuchien  from motadichvu where msdichvu='$mslieutrinh' and mshs='$mshs' and msdv='$msdv' and mscha ='CHA' order by rowid desc");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    public function find_hanghoa($mshs, $msdv, $tensanpham, $msdichvu, $phanbo, $lieutrinh, $loai)
    {
        //Không cho tìm những thằng có mô tả vì khi sum lên trong gói số tiền sẽ lệch mô tả của dịch vụ thêm
        $query = "and lieutrinh = 0 ";
        if ($lieutrinh == '0' && $loai != 'goidichvu') {
            $query .= "and msdichvu not in (select msdichvu from motadichvu where mshs='$mshs' and msdv='$msdv')";
        }
        if ($loai == 'goidichvu') {
            $query = "and lieutrinh <> '2'";
        }
        ...
        SELECT msdichvu AS mshh, tendichvu AS tenhh, sotien AS giaban,'' dvt_quydoi, '' soluong_quydoi, 1 loai, ptthuchien FROM hosodichvu
         WHERE msdv='$msdv' and mshs='$mshs' AND trangthai ='0' AND msdichvu not like '$msdichvu' AND tendichvu like '%$tensanpham%' $query  ORDER BY loai, tenhh");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    ...
    public function delete_chitiet_lieutrinh($rowid, $mshs, $msdv, $msdichvu, $mslieutrinh, $mshh,  $sotien, $phanbo)
    {
        ...
        DELETE FROM motadichvu where mscha='$mshh' and msdichvu='$msdichvu' and mshs='$mshs' and msdv ='$msdv'; 
        $query ");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    ...
    {
        $getall = $this->connect->prepare("SELECT a.mshh, b.lieutrinh,a.mslieutrinh from motadichvu a
        INNER JOIN hosodichvu b ON a.msdichvu = b.msdichvu where a.msdichvu='$msdichvu'  and a.mshs='$mshs' and a.msdv ='$msdv' and a.mscha='CHA' ");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    public function ktra_comota($mshs, $msdv, $msdichvu,  $loai)
    {
        $query = "and mshh='$loai'";
        if ($loai == 'CHA') {
            $query = "and  mscha='CHA'";
        }
        $getall = $this->connect->prepare("SELECT mshh, dongia  from motadichvu where msdichvu='$msdichvu'  and mshs='$mshs' and msdv ='$msdv' $query ");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
    public function add_mota_dichvu($mshs, $msdv, $msdichvu, $mslieutrinh, $mshh, $loai)
    {
        $query = '';
        if ($loai == 'CHA') {
            $query = "and mscha='CHA'";
        }
        $lieutrinh = "'$mslieutrinh'";
        if ($mslieutrinh == '') {
            $lieutrinh = "mslieutrinh";
        }
        $getall = $this->connect->prepare("INSERT INTO motadichvu(lastmodify, msdn, mshs, msdv, msdichvu, mslieutrinh, mscha, mshh, tenhh, soluong, dongia, dinhluong, ptthuchien) SELECT NOW(), msdn, mshs, msdv, '$msdichvu', $lieutrinh, '$mshh', mshh, tenhh, soluong, dongia, dinhluong, ptthuchien FROM motadichvu WHERE mshs='$mshs' AND msdv='$msdv' AND msdichvu='$mshh' $query");
        $getall->setFetchMode(PDO::FETCH_OBJ);
        $getall->execute();
        return $getall->fetchAll();
    }
